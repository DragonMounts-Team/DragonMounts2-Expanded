...
def generateRecipes(output: Output):
    ...
    for type in DragonType:
        ...
        shaped(base.withSuffix('_dragon_scales_block')) \
          .define('X', dragonScales) \
          .pattern('XXX') \
          .pattern('XXX') \
          .pattern('XXX') \
          .unlockedBy('has_scales', unlock) \
          .groupBy('dragon_scales_block') \
          .save(output, "dragon_scales_block") \


        shaped(base.withSuffix('_dragon_scales'),9) \
          .define('X', base.withSuffix('_dragon_scales_block')) \
          .pattern('X') \
          .unlockedBy('has_scales', unlock) \
          .groupBy('dragon_scales_block') \
          .save(output, "dragon_scales")\
